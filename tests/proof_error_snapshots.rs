...
fn fixtures() -> Vec<PathBuf> {
    let dir = Path::new("tests/proof_errors");
...
#[test]
fn proof_error_snapshots() {
    let files = fixtures();
    assert!(
        !files.is_empty(),
        "no .shari fixtures found in tests/proof_errors"
    );
    for path in files {
        ...
        let err = shari::process(file).expect_err(&format!("expected {} to fail", path.display()));
        let formatted = format!("{}\nfixture: {}", format_error(&err), path.display());
